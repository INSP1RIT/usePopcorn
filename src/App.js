        ...
        <Box>
          {selectedId ? (
            ...
            <>
              ...
              <WatchedMoviesList watched={watched} onDeletedWatched={handleDeleted} />
            </>
          )}
        </Box>
      </Main>
...
function NumResults({ movies }) {
  const num_results = movies.length;
  return (
        ...
        <>
          <header>
            ...
            <div className={"details-overview"}>
              <h2>{title}</h2>
              <p>
                {released} &bull; {runtime} IMDb rating
              </p>
              <p>{genre}</p>
              <p>{imdbRating}</p>
            </div>
...
function WatchedMoviesList({ watched, onDeletedWatched }) {
  return (
    <ul className="list">
      {watched.map((movie) => (
        <WatchedMovie movie={movie} onDeletedWatched={onDeletedWatched} />
      ))}
    </ul>
  );
}
  ...
  return (
    <li key={movie.imdbID}>
      ...
      <div>
        ...
        <p>
          <span>🌟</span>
          <span>{movie.userRating.toFixed()}</span>
        </p>
        <p>
          <span>⏳</span>
